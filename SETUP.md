...
Freebuff is the AI agent that powers this job search framework. Visit [freebuff.com](https://freebuff.com) to get started.

> Note: This framework was originally built for Claude Code but has been adapted to work with Freebuff (Buffy).
...
By default, MiKTeX installs missing packages on demand but pops up a GUI prompt for each one — which blocks non-interactive terminals. Turn that into a silent auto-install instead:
...
Start Freebuff in the repository and run the onboarding:

```
/setup
```

Freebuff will offer three paths:

- **Path A (documents folder):** Add your CV, LinkedIn export, diplomas, references, or past applications under `documents/`. Freebuff reads and cross-references them before proposing profile updates. This is best when you have several source files.
- **Path B (single CV import):** Share one CV/resume by mentioning the file with `@` or pasting the text. Freebuff extracts it and asks follow-up questions for anything missing.
...
Shared permissions now live in `.claude/settings.json` (scoped to `bun run`, `python salary_lookup.py`, and `python3 salary_lookup.py`). Earlier versions of this repo committed a broader `.claude/settings.local.json` that pre-approved `Bash(curl:*)`, `Bash(python:*)` and `Bash(bun:*)`. If you cloned before that change, git leaves the old file behind in your working copy, and its permissions still apply on top of `settings.json`. Delete it (or trim it to your own personal overrides):
